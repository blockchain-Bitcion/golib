...
namespace godapp {
    ...
    void centergame::transfer(name from, name to, asset quantity, string memo) {
        ...
        if (!check_validTime(message,timestamp)) {
            return;
        }else{
    ...
    void centergame::reveal(uint64_t game_id, const std::string& message, const std::vector<uint64_t>& bet_ids,
        ...
        payment_map pay_map;
        name empty_referer;
        eosio_assert(bet_ids.size() != 0, "Empty bet result");
        eosio_assert(bet_ids.size() == prize_amounts.size(), "Result size does not match");
